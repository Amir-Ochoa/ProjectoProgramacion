...
public class UsuarioInerfaz extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        Label academiasLabel = new Label("Usuario");
        ...
        TableColumn<Usuario, String> correoColumn = new TableColumn<>("Correo");
        TableColumn<Usuario, String> contrasenaColumn = new TableColumn<>("Contraseña");
        ...
        contrasenaColumn.setCellValueFactory(new PropertyValueFactory<>("Contraseña"));

        // Añadir las columnas a la tabla
        table.getColumns().addAll(correoColumn, rolColumn);

        // Agregar filas de ejemplo a la tabla

        UsuarioHelper uh = new UsuarioHelper();
        List<Usuario> listaAcademias = uh.obtenerListaUsuario();

        // Crear los datos de ejemplo
        // Convierte la lista de academias en una ObservableList
        ObservableList<Usuario> observableListaUsuario = FXCollections.observableArrayList(listaAcademias);

        // Asigna la ObservableList a la tabla
        table.setItems(observableListaUsuario);



        table.setPrefWidth(900); // Establece el ancho preferido de la tabla
        ...
        experienciasButton.setOnAction(e-> mostrarVentanaExperiencia(primaryStage));

        filtrarButton.setOnAction(e -> {
            String nombreBuscado = buscarField.getText().trim();
            if (!nombreBuscado.isEmpty()) {
                List<Usuario> academiasFiltradas = listaAcademias.stream()
                        .filter(usuario -> usuario.getCorreo().toLowerCase().contains(nombreBuscado.toLowerCase()))
    ...
    private void mostrarVentanaAcademia(Stage currentStage) {
        currentStage.close();
        Stage acd = new Stage();
        InterfazAcademia ventanaAcademia = new InterfazAcademia();
        ventanaAcademia.start(acd);

    }

    private void mostrarVentanaExperiencia(Stage currentStage) {
        currentStage.close();
        Stage acd = new Stage();
        InterfazEE ventanaAcademia = new InterfazEE();
        ventanaAcademia.start(acd);
